...
class molecule():    
    ...
    def __init__(self,mol):        
        ...
        self.unique_atom_keys, self.unique_atom_indexes, self.unique_atom_inverse = dummy 
        ...
        assert np.array_equal(self.unique_atom_keys[self.unique_atom_inverse],self.atom_names),"unique atom in inverse doesnt work" 
        
        dummy = unique_sort(self.bond_keys,return_inverse=True)
        self.unique_bond_keys, self.unique_bond_indexes, self.unique_bond_inverse = dummy 
        ...
        if len(self.bond_keys) > 0:
                assert np.array_equal(self.unique_bond_keys[self.unique_bond_inverse],self.bond_keys),"unique bond in inverse doesnt work" 
        
        dummy = unique_sort(self.angle_keys,return_inverse=True)
        self.unique_angle_keys, self.unique_angle_indexes, self.unique_angle_inverse = dummy 
        ...
        if len(self.angle_keys ) > 0:
                assert np.array_equal(self.unique_angle_keys[self.unique_angle_inverse],self.angle_keys),"unique angle in inverse doesnt work" 
        
        dummy = unique_sort(self.torsion_keys,return_inverse=True)
        self.unique_torsion_keys, self.unique_torsion_indexes, self.unique_torsion_inverse = dummy 
        ...
        if len(self.torsion_keys ) > 0:
                assert np.array_equal(self.unique_torsion_keys[self.unique_torsion_inverse],self.torsion_keys),"unique angle in inverse doesnt work" 
        ...
        for i,a0 in enumerate(self.unique_atom_names):
            for j,a1 in enumerate(self.unique_atom_names):
                if i>j:
                    ...
                    self.unique_atom_pair_keys.append( make_graph(self.unique_atom_pair_names[-1]) )
    ...
    def get_neighbour_list(self):
        ...
        for i,fi in zip( f_p , self.f[f_p] ):
            print( branches )
            if fi < 0: 
                idx = np.max(get_diff_in_bond_lists( f_p[f_p < i], np.arange(i) )) # index of last atom
                branches[i] = get_branch_root( branches[idx], branches )  # connect ring to acompanying branch
                ...
                branch = branches[i]
                p_subchain_i = np.atleast_1d(np.squeeze( np.where( branches == branch ) ))[1:]
    ...
    def get_distance_matrix(self):
        ...
        """
        self.distance_matrix, self.bond_lists = get_distance_matrix(self.neighbour_list,self.atom_number)
        self.bond_list = self.bond_lists[0]
        self.bond_names = np.array([ sort_force_fields(self.atom_names[a]) for a in self.bond_list ])
        ...
        if len(self.bond_lists) > 1:
            ...
            self.angle_names = np.array([ sort_force_fields(self.atom_names[a]) for a in self.angle_list ])
        ...
        if len(self.bond_lists) > 2:
            ...
            self.torsion_names = np.array([ sort_force_fields(self.atom_names[a]) for a in self.torsion_list ])
    ...
    def map_unique_via_atom_names(self, data):        
        ...
        Returns:
            mapped unique data
        """
        print("map_unique_via_atom_names is outtdated... dont use\n shouldnt cause errors btw")
